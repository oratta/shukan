...
| light ベール（ホーム） | `.banner-veil` / 展開時 `.banner-veil-expanded`（全面 color-mix グラデ＋blur） |
| light 軽ベール（入口/発見） | `.banner-veil-browse`（下部だけ color-mix ウォッシュ・blur なし） |
...
**入口画面（発見など写真が主役の browse）は `.banner-veil-browse` を使う。** ホーム用 `.banner-veil` は
全面フロスト（blur＋均一ウォッシュ）で、写真を脇役に落とす前提。これを写真がタップ対象そのものである
入口画面に流用すると、全カードが washed out な均一グレーになり「押せない板」に見える（実機フィードバック）。
`.banner-veil-browse` は blur を持たず、テキストが乗る下部だけ背景色でウォッシュし上部は写真を素通しにする
（写真の彩度を殺さない＝タップできる写真タイルに見える）。文字は同じくインク（`text-foreground`）で、
下端の濃いウォッシュで可読性を確保する。dark 側は入口でもホームと同じ `.banner-scrim`（黒スクリム）を使う。
