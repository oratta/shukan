...
- **config.yaml rules**:
  ...
  - "`app/robots.ts` は dynamic 実装。`headers()` から host を読み、`www.s-mitch.com` の場合のみ marketing 配下を Allow。apex の場合は既存挙動維持"
  - "`app/sitemap.ts` も同様に host 判定して出し分け"
...
12. [ ] `app/robots.ts` が dynamic 実装で、host = www のときに marketing を Allow、host = apex のときに既存挙動を返す（テストはホスト別に Vitest で出力検証）
