...
- **API route 配置とミドルウェア整合（採用方針）**: Webhook は `/api/stripe/webhook`（middleware matcher 外、handler 内で署名検証）。認証必須の Checkout/Portal セッション生成 route は `/api/stripe/*` に置き、route handler 内で `supabase.auth.getUser()` により認可する。**middleware matcher は拡張しない**（Webhook と認証 route の matcher 管理混在を避ける）
...
- **残り枠カウンタの公開方針**: anon 取得可能な公開エンドポイント（集計値のみ返す。個人データ・user_id 等は非露出）。短期キャッシュ（10〜30秒 revalidate）で実数を返す。ティザー（未ログイン）からの参照と景表法の実数表示要件（change-D）の両方を満たす
...
- **i18n 実装方式（採用方針）**: ティザーは next-intl（`src/messages/{en,ja}.json` に `founding` 名前空間を追加）で en/ja 対応する。既存 `src/app/marketing/copy.ts` の静的日本語エクスポート方式は踏襲しない（waitlist の locale 保存・将来の課金導線と cookie locale の一貫性が必要なため）
- **waitlist の RLS / 濫用対策（採用方針）**: `to anon` で insert のみ許可。select は service_role のみ（anon select 不可）。email 形式バリデーション + `unique(email)` で重複弾き。同一 email の再 insert は upsert で無害化（簡易 rate limit 相当）。※ anon insert はコードベース初の前例になるため、マイグレーションにコメントで意図を明記
...
- **スコープ**:
  ...
  - **個人情報保護法**: waitlist/アカウントのメール取得にプライバシーポリシー整備・同意。**注: `/privacy` と `/terms` ページは既存（`src/app/privacy/`, `src/app/terms/`）。新規作成ではなく、既存ページに waitlist のメール取得・課金（Stripe への決済情報送信）に関する条項を追記する**
