...
## 🟢 プロト検証中（2026-07-01）— 精査済み15習慣で量チェック
- プロト `prototype/onboarding-step2-proto.html` に**精査済み15習慣のみ**を実データで流し込み・段階タップUXを実装（配信 `python3 -m http.server` → ブラウザ）。
  - 内訳: 健康9（タバコ/運動/睡眠/野菜/水/砂糖/ジャンク/筋トレ/発酵食品）／前向き5（瞑想/日記/自然/社会的つながり/朝の光）／出費1（アルコール）／収入0。
  - **未精査のcost/earning束ね（自炊/集中して働く/朝を整える/学び続ける/節約/衝動買い）はプロトから除外**（ユーザー指示: 精査OK分のみ表示・不足なら追加）。※source presets には残置。
- UX確定: 段階タップは**2×2グリッド**（縦4バーをやめてファーストビュー収まり達成）、質問+選択肢は最下部固定・上部のみスクロール。
- **筋トレ頻度をエビデンス準拠に修正**: 「毎日」→「週2〜3回」（Momma 2022 で飽和）。presets/messages/proto/effect-model §8 反映済み。
- レベル『だいたい』サブを頻度非依存「ほとんどできている」に（2日に1回等の非daily習慣にも通用）。
- 次: ユーザーが量・文言をチェック → 不足なら earning/cost 系の精査・追加を再開。**アプリ本体オンボの「精査分のみ表示」への絞り込みは未対応**（プロト先行）。
