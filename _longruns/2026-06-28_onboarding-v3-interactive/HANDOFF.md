...
### ▶ 次にやること（このセッションの続き）
**習慣④「体にいい食事」の中身チェック**から再開する。論点:
- 束ね `eat_vegetables` + `drink_water` + `quit_sugar` のウェイト（合算か内包ブレンドか）と文言を確定。3本束ねなのでここは②③と違い「別物の積み上げ vs 重複」の見極めが要る。
- 以降の earning/cost/mood 系プリセットも1つずつ**文言・束ねエビデンス・ウェイト**を確定していく。
