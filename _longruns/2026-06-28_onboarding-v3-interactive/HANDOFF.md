...
### ▶ 次にやること（このセッションの続き）
**習慣②「体を動かす」の中身チェック**から再開する。論点:
- 束ねエビデンス `daily_cardio` + `daily_walking` が weight 100+100 の単純合算＝**二重計上ぎみ**（同じ有酸素運動）。
  ウェイトを振るべきか（例 cardio70/walking30）を確定する。
- 以降 ③しっかり眠る（sleep+寝る前スマホ） ④体にいい食事（野菜+水+砂糖断ち）… と、習慣を1つずつ
  **文言・束ねエビデンス・ウェイト**を確定していく。`src/data/habit-presets.ts` が現行の束ね定義。
