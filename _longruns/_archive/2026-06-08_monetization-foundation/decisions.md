...
## D12: /account ランタイム FORMATTING_ERROR 修正 — プレースホルダー付きメッセージは raw テンプレートで渡す（2026-06-12）

ユーザー報告の `/account` ランタイムエラー（`FORMATTING_ERROR: The intl string context variable "price" was not provided to the string "{price} / year (billed annually)"`）の修正。`page.tsx` が checkout/account 名前空間の ICU プレースホルダー付きキー（`{price}/{plan}/{days}/{total}/{remaining}/{cap}`）を補間値なしの `t(key)` で解決していたため、next-intl が実行時に throw していた。下流ビュー（AccountBilling / FinalConfirmation）は元々これらを raw テンプレートとして受け取り `pricing.ts`（税込総額の単一ソース・表示額=請求額の不変条件）で自前補間する設計。したがって page は raw テンプレートを渡すべき。修正は props 組み立てを純粋関数 `buildAccountBillingMessages`（`src/app/(app)/account/account-messages.ts`）へ抽出し、プレースホルダー付きキーは `t.raw(key)`、それ以外は `t(key)` で解決。回帰テスト `account-intl-values.test.ts` は実 messages を `createTranslator` で走査し、全プレースホルダーキーが raw `{var}` テンプレートとして props に残ることを検証（環境非依存・モック不要、修正前は RED）。検証: `npm run test:run` → 390 passed / `npx tsc --noEmit` → 編集ファイル新規エラーゼロ / `npm run lint` → 9 errors / 36 warnings（ベースライン維持）/ `npm run build` → 成功 / dev 3001 で `/account` 未認証 → `/login` へ 307 リダイレクト確認。

## D13: founding ティザーの self-fetch 廃止 — `getFoundingCounts()` 直接呼び出しへ（Feedback リファクタ, 2026-06-21）

ユーザー報告の `GET /api/founding/slots 503`（実体は `SUPABASE_SERVICE_ROLE_KEY` 未設定で `.env.local` 設定済み・解決済み）を契機に判明した設計上のアンチパターンの根本対処。`/founding` ティザー（**サーバーコンポーネント**）が自分自身の API ルート `/api/founding/slots` を HTTP で **self-fetch** していた。サーバー側の相対 fetch は origin を解決できず `FOUNDING_COUNTER_API_URL` / `NEXT_PUBLIC_APP_URL` の絶対 URL env に依存し、ポート不一致で壊れやすい。当初は「change-B（counter API）未デプロイ時の分離点」として設計（D5/D10）したが、**change-B は既に同一アプリにマージ済み**なので間接化は不要。

- **判断**: `src/app/founding/page.tsx`（サーバーコンポーネント）を `getFoundingCounts()`（`@/lib/supabase/founding-admin`, `server-only`, service-role）の**直接呼び出し**に変更。HTTP self-fetch（`fetchRemainingSlots`）を廃止。`getFoundingCounts()` を `loadRemainingSlots()` の try/catch で囲み、失敗時は `null`（env 未設定・DB 不通でも偽の数値は絶対に出さない「数値非表示フォールバック」セマンティクスを完全維持＝change-C D5 / 景表法 D7-5）
- **キャッシュ契約（10〜30秒）の維持方法 — `export const revalidate = 15` を選択**: ページ全体のレンダーを route-segment revalidate で 15 秒キャッシュ（change-B の 10〜30 秒契約に整合）。**代替案**: (A) `unstable_cache(getFoundingCounts, {revalidate:15})` でラップ → `unstable` API、かつ `server-only` 関数をラップする複雑さが増す（YAGNI 違反）。(B) `route.ts` の `Cache-Control` のみに依存 → ページが API を呼ばなくなったので無関係。可逆かつ最もシンプルな `revalidate=15` を採用。build で `/founding` は `ƒ (Dynamic)` server-rendered on demand（service-role env を実行時参照するため static prerender されないが、revalidate=15 でリクエスト間キャッシュされる）
- **`account/page.tsx`（client）は不変更**: ブラウザ実行のため相対 `/api/founding/slots` fetch が origin 解決でき env 不要。`fetchRemainingSlots()` のまま正しい
- **`src/app/founding/slots.ts` の `resolveCounterUrl()` 削除・相対パス専用化**: もはや client（account ページ）からの相対 fetch 専用なので `FOUNDING_COUNTER_API_URL` / `NEXT_PUBLIC_APP_URL` の絶対 URL 解決を撤去し、固定の相対パス `/api/founding/slots` を fetch するだけにした。`fetchRemainingSlots` 自体は account client が使うので残置。ファイル冒頭コメントを現状（client account 専用 HTTP フェッチャー、teaser は直接呼び出しへ移行済み）に更新
