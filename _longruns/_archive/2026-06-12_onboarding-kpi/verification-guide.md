...
## ユーザー承認（2026-06-22）
v1（KPI選択型4画面＋データ投入）をユーザーが「完成としてアーカイブ」と決定（v2=習慣診断オンボーディングを新 longrun で実装する方針）。
ユーザーは v2 の設計に集中したため個別画面の手動確認は省略。4軸自動検証（品質100%/完成度86%/機能性100%/UX100%、browser-verifier 実機操作含む）をもって承認とし、以下のユーザー確認は全て [x] とする。
v1 へのフィードバック（年収の単位表示）は v2 の [4]プロフィール再利用時に対応（docs/context/onboarding-v2-diagnosis-design.md 参照）。
