...
- **config.yaml rules**:
  ...
  - "トリガーは『通常習慣の setDayStatus で completed 遷移』と『quit 習慣の markQuitDailyDone 完了』の両完了経路を対象とする。判定は habit-card.tsx の prevStatusRef パターン（非completed→completed の遷移検出）に倣ってよい"
