...
- アプリ全体UIの再構築: 外部リリースに向け、オンボーディング（onboarding-kpi run）で確立した新しいデザイントーンに合わせてアプリ全体（ホーム含む）をわかりやすく整理されたUIに刷新する（ホームでの「選んだKPI（tracked_kpis）」の表示・強調は 2026-07-03 impact-3scene run で先行対応済み）
