...
- [x] 6.2 ブラウザ検証: タップ二値トグル（positive / quit / weekly / 過去日ドット）、長押しシート（失敗→チップ→保存、チップ無視で failed 維持、スキップ、メモ）、失敗日の反転塗り表示、既存 quit 習慣（urge_logs 残存）の表示が壊れないこと、ドラッグ&ドロップが引き続き動くこと
- [x] 6.3 受け入れ条件（issue #104）の9項目を全チェックする
