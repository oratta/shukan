...
システムは、ステータスボタンの長押し（500ms）で今日のアクションシートを開かなければならない（MUST）。過去日の週ドットは**タップ一発**でその日付のアクションシートを開かなければならず（MUST）、週ドットにトグル・長押しの操作を割り当ててはならない（MUST NOT）。シートの構成は positive / quit で共通でなければならない（MUST）。
...
#### Scenario: 過去日の週ドットはタップでシートが開く
- **WHEN** 過去日の週ドットをタップする
- **THEN** その日付を対象としたアクションシートが開く（ドットのタップでステータスは変化しない）
