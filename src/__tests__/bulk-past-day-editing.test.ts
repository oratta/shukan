...
// - 編集可能枠・自動失敗表示・ロケット境界の3つを EDITABLE_PAST_DAYS(=6) に統一
//   （今日を含めて計7日間＝1週間。週次習慣の時間単位と揃える）
...
describe('EDITABLE_PAST_DAYS 定数', () => {
  it('編集可能枠は過去6日（今日を含めて1週間）である', () => {
    expect(EDITABLE_PAST_DAYS).toBe(6);
  });
});

describe('getEditablePastDays: 対象日の決定', () => {
  it('毎日習慣は今日を除く過去6日を新しい順で返す', () => {
  ...
  it('weekday 習慣は月〜金の日だけを返す', () => {
    ...
    const days = getEditablePastDays(habit, []);
    // 過去6日に含まれる平日数は曜日次第（4または5日）なので動的に期待値を組む
    const expected = [1, 2, 3, 4, 5, 6]
      .map((n) => dayOfWeekOf(daysAgo(n)))
      .filter((dow) => dow >= 1 && dow <= 5).length;
  ...
  it('custom 習慣は指定曜日の日だけを返す', () => {
    ...
    const days = getEditablePastDays(habit, []);
    // 過去6日のうち特定曜日（今日と別曜日）はちょうど1日
    expect(days).toHaveLength(1);
    expect(days[0].date).toBe(daysAgo(3));
  });

  it('weekly 習慣は過去6日すべてを返す', () => {
...
describe('editablePastDays: 週ドットと一括編集シートの行の1:1一致', () => {
  ...
  it('曜日指定習慣でも editablePastDays は過去6日の対象曜日のみ（シート行と同じ）', () => {
    const targetDow = dayOfWeekOf(daysAgo(3));
    const habit = makeHabit({ id: 'h1', frequency: 'custom', customDays: [targetDow] });
...
describe('自動失敗表示の境界統一（getAllDayStatuses 経由の allDays）', () => {
  it('編集可能枠内（6日前まで）の未記録日は none、7日以上前は failed', () => {
    ...
    expect(byDate.get(daysAgo(7))).toBe('failed');
...
describe('getEffectiveStatus の境界統一', () => {
  it('6日前までの未記録は none、7日以上前は failed', () => {
