...
describe('computeDiagnosisV3（プリセット×達成率の集計・未来のみ）', () => {
  it('タバコ 100%（V2デフォルトプロフィール=残り寿命40年）', () => {
    ...
    // mood: タバコの per-day mood=72（4KPI化: 禁煙のメンタルヘルス改善 Taylor et al., 2014）
    expect(result.byKpi.positive_mood).toEqual({ raw: 72, display: '72', unit: '分/日' });
  ...
  it('複数習慣は KPI ごとに加算される', () => {
    ...
    // mood: タバコ72 + 運動72 = 144 分/日
    expect(result.byKpi.positive_mood.raw).toBe(144);
  });
...
describe('rankPresetsByGrowth（伸びしろ = (1-達成率) × 100%ポテンシャル）', () => {
  ...
  it('4KPI化後は全プリセットが positive_mood にも効果を持つ（raw=0 除外分岐はデータ上発生しない）', () => {
    // 全記事4KPI化により per-day mood=0 の習慣は現行データに存在しない。
    // rankPresetsByGrowth の raw>0 フィルタは将来のデータ向けの防御分岐として残る。
    expect(habitPotentialV3('quit_smoking_for_health', null).byKpi.positive_mood.raw).toBeGreaterThan(0);
    const ranked = rankPresetsByGrowth('positive_mood', {}, null, 15);
    for (const c of ranked) {
      expect(habitPotentialV3(c.presetId, null).byKpi.positive_mood.raw).toBeGreaterThan(0);
    }
  });
