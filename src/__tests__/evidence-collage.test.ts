...
describe('F17: 展開時もコラージュ背景を維持し、緑枠を廃して写真前提に調整', () => {
  ...
  it('ImpactBadge / SavingsCard に写真背景用 surface を渡す', () => {
    expect(card).toContain("surface={hasEvidenceBg ? 'onImage' : 'default'}");
  });

  it('ImpactBadge / SavingsCard は onImage で緑ボーダーを白ガラスに切替える', () => {
    expect(badge).toContain('onImage');
    expect(badge).toContain('border-white/20');
    expect(savings).toContain('onImage');
