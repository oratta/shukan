 ...
 *      ログイン中の変更は必ず DB へ即書き込まれる（useSettings.saveTheme / saveLocale）。
 *
 *      【不変条件は「カラム単位」の last-writer-wins（#101 で訂正）】
 *      書き込みは**変更したカラムだけ**を送る（`updateUserSettings`）。したがって DB が保持するのは
 *      「そのアカウントで **そのカラムに対して** 最後に行われた変更」であって、
 *      「行に対して最後に行われた変更」ではない。
 *      この区別が必要なのは、取り込み（`useSettingsSync`）が userId ごとに 1 回きりで realtime 購読が
 *      無く、**開きっぱなしのタブのローカル値は他デバイスの変更に対して stale になりうる**ため。
 *      行まるごと書くと、ユーザーが触っていないカラムを stale なローカル値で巻き戻してしまう
 *      （端末 A で言語だけ変更 → 端末 B の古いタブでテーマだけ変更 → 言語が巻き戻る、が決定論的に起きた）。
 *      カラム単位で書けば、触っていないカラムには誰も書かないので他デバイスの変更が保たれる。
